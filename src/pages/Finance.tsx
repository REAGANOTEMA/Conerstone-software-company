...
const Finance = () => {
  ...
  const handleAddTx = () => {
    ...
    const updated = storage.add('transactions', { ...newTx, id, amount: parseFloat(newTx.amount) });
            ...
            <TableBody>
              {transactions.map((tx) => (
                <TableRow key={tx.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={tx.type === 'Revenue' ? 'text-emerald-600 border-emerald-200' : 'text-red-600 border-red-200'}>
                      {tx.type}
                    </Badge>
                  </TableCell>
                  <TableCell className="font-bold">
                    {tx.type === 'Revenue' ? '+' : '-'}{formatUGX(tx.amount)}
                  </TableCell>
                  <TableCell>
                    <Badge className="bg-slate-50 text-slate-600 border-none">
                      {tx.status}
                    </Badge>
                  </TableCell>
        ...
        <DialogContent className="sm:max-w-[500px] rounded-3xl">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input value={newTx.client} onChange={e => setNewTx({...newTx, client: e.target.value})} placeholder="e.g. AWS or Iganga High" className="rounded-xl" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input type="number" value={newTx.amount} onChange={e => setNewTx({...newTx, amount: e.target.value})} placeholder="0" className="rounded-xl" />
              </div>
            </div>
          </div>
          <DialogFooter>
